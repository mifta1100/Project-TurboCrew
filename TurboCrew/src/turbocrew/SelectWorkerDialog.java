...
public class SelectWorkerDialog extends HBox{
    TableView table;

    public SelectWorkerDialog(AddAssignment addAssignment) {
        ...
        selectButton.setOnAction((ActionEvent e) -> {
            try{
                Worker worker = (Worker)table.getSelectionModel().getSelectedItem();
                addAssignment.setWorker(worker);
                addAssignment.setWorkerFields();
                if(worker == null)
                    throw new Exception();
            }catch(Exception ex){
            ...
            Stage stage = (Stage) SelectWorkerDialog.this.getScene().getWindow();
            stage.close();
        });
